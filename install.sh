...
#
# Builds the server from THIS checkout and boots it with Postgres, and opens the
# firewall ports that LAN autodiscovery needs. Use this when you are installing
# from a working tree; to install the released build instead, nothing needs to
# be cloned at all:
#
#   docker compose -f oci://ghcr.io/bouliehaan/samo-server:compose up -d
#
# The server runs with host networking (see docker-compose.yml), so it listens
...
if [ "${DO_UP}" -eq 0 ]; then
  note "firewall done. Bring the stack up with:  ${COMPOSE[*]} -f docker-compose.yml -f docker-compose.build.yml up -d --build"
  exit 0
fi

note "building from this checkout and starting the stack..."
( cd "${SCRIPT_DIR}" && "${COMPOSE[@]}" -f docker-compose.yml -f docker-compose.build.yml up -d --build )
