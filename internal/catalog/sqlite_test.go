...
func TestLoadSeedFromDBHydratesMusicAndAudiobooks(t *testing.T) {
	...
	exec(`INSERT INTO libraries (id, name, kind, path) VALUES ('library-1', 'Audiobooks', 'audiobook', '/books')`)
	exec(`INSERT INTO audiobooks (id, library_id, path, duration_seconds, book_json) VALUES ('book-1', 'library-1', '/books/Ada/Signal Manual', 7200, '{"title":"Signal Manual","externalIds":{"audibleAsin":"B000SAMO"}}')`)
	...
	if len(seed.Audiobooks) != 1 || seed.Audiobooks[0].Book == nil {
		t.Fatalf("audiobooks = %#v, want one book item", seed.Audiobooks)
	}
	if seed.Audiobooks[0].Book.ExternalIDs.AudibleASIN != "B000SAMO" {
		t.Fatalf("Audible ASIN = %q, want B000SAMO", seed.Audiobooks[0].Book.ExternalIDs.AudibleASIN)
	}
}
