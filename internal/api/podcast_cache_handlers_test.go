...
func TestClearPodcastCacheRemovesAllRows(t *testing.T) {
	...
	cachePath := filepath.Join(cacheService.CacheDir(), "ep-1.mp3")
	...
	if _, err := db.ExecContext(ctx, `
		...
		VALUES ('ep-1', 'https://example.com/ep-1.mp3', ?, 'audio/mpeg', 6, ?, ?)`,
		cachePath, now, now); err != nil {
		t.Fatal(err)
	}

	handler := NewServer(ServerOptions{PodcastCache: cacheService})

	req := httptest.NewRequest(http.MethodDelete, "/api/v1/podcasts/cache", nil)
	...
	cached, ok, err := cacheService.Lookup(ctx, "ep-1", "https://example.com/ep-1.mp3")
	if err != nil || ok || cached.Path != "" {
		t.Fatalf("lookup after clear ok=%v err=%v", ok, err)
	}
}
